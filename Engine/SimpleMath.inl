...
inline void Vector3::Abs(Vector3& result) const
{
	using namespace DirectX;
...
//this appears to be incorrect, the result of this and perspective matrix is almost identity but the last row is wrong
inline Matrix Matrix::CreateInversePerspectiveFieldOfView(float fov, float aspectRatio, float nearPlane, float farPlane)
{
	using namespace DirectX;
	float    SinFov;
	float    CosFov;
	...
	XMMATRIX R;
	XMVECTOR row0 = { 1 / Width, 0.0f, 0.0f, 0.0f };
	R.r[0] = row0;

	XMVECTOR row1 = { 0.0f, 1 / Height, 0.0f, 0.0f };
	R.r[1] = row1;

	XMVECTOR row2 = { 0.0f, 0.0f, 0.0f, 1 / (-fRange * nearPlane) };
	R.r[2] = row2;

	XMVECTOR row3 = { 0.0f, 0.0f, 1.0f, -(fRange) / (1.0f *  -fRange * nearPlane) };
	R.r[3] = row2;
...
//SRT matrix - custom
inline void Matrix::SRT(const Vector3& scale, const Quaternion& rotation, const Vector3& translation, Matrix& result)
{
	using namespace DirectX;
	XMVECTOR transv = XMLoadFloat3(&translation);
	XMVECTOR rotv = XMLoadFloat4(&rotation);
	XMVECTOR scalev = XMLoadFloat3(&scale);
	XMMATRIX M = XMMatrixAffineTransformation(scalev, XMVectorZero(), rotv, transv);

	XMStoreFloat4x4(&result, M);
}

//SRT matrix - custom
inline Matrix Matrix::SRT(const Vector3& scale, const Quaternion& rotation, const Vector3& translation)
{
	using namespace DirectX;
	XMVECTOR transv = XMLoadFloat3(&translation);
	XMVECTOR rotv = XMLoadFloat4(&rotation);
...
//this is rhs, then lhs, so rhs applies to world space then lhs applies over it
//https://math.stackexchange.com/questions/2124361/quaternions-multiplication-order-to-rotate-unrotate
//Multiplication by quaternions in the conventional way transforms the world coordinates of whatever object you apply it to.
//If you apply the child rotation to the world coordinates, then apply the parent transform to the result, it is like we rotated to the parents
//frame of reference, then applied the child rotation. This is because multiplying by the parents quaternion changed the world coordinates of the result
//of the child rotation, bring it into the parents local space. With this logic, we imply that the lhs occurs first, then the rhs. 
//However this is non-intuitive, because matrix rotations happen in the opposite order. To this end it seems like DirectX reverses the order for convenience. 
inline Quaternion operator* (const Quaternion& Q1, const Quaternion& Q2)
{
	using namespace DirectX;
	XMVECTOR q1 = XMLoadFloat4(&Q1);
...
// String function
//------------------------------------------------------------------------------
inline void Quaternion::ToString()
{
	std::stringstream ss;
	ss << "{" << this->x << ", " << this->y << ", " << this->z << ", " << this->w << "}" << std::endl;
	//const std::string temp = ss.str();
	////ss << "Hi my name is jack\n";
	OutputDebugStringA(ss.str().c_str()); 
	//std::string temp = std::string("{") + std::to_string(this->x);// +",";
	//return temp;
}
...
//this function is not convenient, it doesnt convert from degrees to radians and the order of input parameters is y,x,z which feels awkard
...
//takes euler angles in degrees and returns a quaternion (x,y,z)
inline Quaternion Quaternion::CreateFromPitchYawRoll(float pitch, float yaw, float roll)
{
	using namespace DirectX;
	Quaternion R;
...
//[https://www.dropbox.com/s/50j8r981modx669/Quaternion%20Sequences.png?dl=0] self made explanation of our choices here
//we rotate roll, then pitch then yaw same as the DXMath Library
//however wikipedia has a different coordinate axis, they also roll then pitch then yaw, but the roll/pitch/yaw is encoded differently into the
//quaternion to reflect their coordinate axis, thus we must rearrange our quaternion to use their conversion
inline Vector3 Quaternion::EulerAngles(const Quaternion& q1)
{
	using namespace DirectX;
	Quaternion q = { q1.z, q1.x, q1.y, q1.w };
	...
	const float singularityCheck = 2.0f * (yw - xz);
	const float singularityThreshold = 0.999999f;
	//const float singularityThreshold = 0.99f;
	const float rad2deg = 180.0f * XM_1DIVPI;

	//yaw, roll, blank, blank
	Vector4 atan2vec1 = { 2.0f * (zw + xy), 2.0f * (xw + yz), 1, 1 };
	Vector4 atan2vec2 = { 1.0f - (2.0f * (yy + zz)), 1.0f - (2.0f * (xx + yy)), 1, 1 };
	Vector4 atan2result = XMVectorATan2(atan2vec1, atan2vec2);

	float pitch;
	float yaw;
	float roll;

	//check for singularity at north and south pole
	if (fabs(singularityCheck) >= singularityThreshold)
	{
		pitch = copysign(90.0f, singularityCheck); // use 90 degrees if out of range
	}
	else
	{
		//pitch = XMScalarASinEst(singularityCheck) * rad2deg;   //the approximation seems good enough for what we need
		pitch = XMScalarASin(singularityCheck) * rad2deg;
	}
	yaw = atan2result.x * rad2deg;
	roll = atan2result.y * rad2deg;
